...
// @version      2.0-beta2
// @description  Painel com Grade Curricular, Sidebar e correção de bugs visuais.
// @author       Gusttavo
...
(function() {
    ...
    // bloqueia o script se estiver na tela de login ou pagina publica
    ...
    // descobre se estamos no painel principal ou numa pagina interna (tipo notas)
    ...
    // funcoes auxiliares pra arrumar o nome do aluno
    ...
    // data de hoje para o card de boas vindas
    ...
    // recupera dados salvos ou cria vazios
    let aluno = JSON.parse(localStorage.getItem('sigaa_plus_cache') || '{}');
    let carteirinha = { matricula: "---", curso: "---", nivel: "---", status: "---", email: "---", entrada: "---" };

    // se tiver no painel, tenta ler os dados da pagina
    if (isDashboard) {
        try {
            // esconde o layout antigo na força bruta (correcao do bug fantasma)
    ...
    // cria os badges de horario nos cartoes das materias
    ...
    // le o texto da turma e converte em dados pra grade
    ...
    // monta o html do modal da grade
    function criarModalGrade() {
        ...
        // configuracoes de horario (fixo 07h as 22h)
        const startHour = 7;
        const endHour = 22;
        ...
        // desenha as linhas horizontais da tabela
        let gridLinesHTML = '';
        ...
        // desenha os numeros da hora na esquerda
        let timeLabelsHTML = '';
        for(let h = startHour; h < endHour; h++) {
        ...
        let daysColsHTML = diasSemana.map(dia => {
            ...
            let blocosHTML = aulasDia.map(aula => {
                ...
                // calcula posicao e altura em porcentagem
        ...
        // monta a estrutura final do modal
    ...
    // se estiver no painel, carrega sidebar e cards
    if (isDashboard) {
        GM_addStyle(`
        ...
        // cria os cards das materias lendo a tabela
        ...
        // gambiarra para arrumar o menu nativo do sigaa que quebra
        ...
        // estilos para paginas internas (notas, frequencia, etc)
        GM_addStyle(`
