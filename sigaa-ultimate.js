...
// @match        *://*.sigaa.ufc.br/*
// @match        *://*.si3.ufc.br/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=ufc.br
// @grant        GM_addStyle
// @grant        GM_getValue
// @grant        GM_setValue
// @run-at       document-end
...
(function() {
    ...
    if (url.includes('logar.do') || url.includes('/sigaa/verTelaLogin.do') || document.querySelector('input[name="user.login"]')) {
        function carregarLogin() {
            'use strict';

    const SELECTORS = {
        configBtn: 'btn-config',
        themeModal: 'modal-temas',
        closeModalBtn: 'btn-close-modal',
        themeBtn: '.theme-btn',
        loginForm: 'loginFormMask',
        maskLogin: 'mask_login',
        maskSenha: 'mask_senha',
        togglePassBtn: 'togglePassBtn',
        eyeIcon: 'eyeIcon',
        submitBtn: 'btn-entrar',
        sigaaLoginContainer: 'sigaa-new-login',
        originalLoginForm: 'input[name="user.login"]',
        originalPassForm: 'input[name="user.senha"]',
        originalSubmitBtn: 'input[type="submit"][value="Entrar"]'
    };

    const CONFIG = {
        themeKey: 'sigaa_palette',
        cacheKey: 'sigaa_plus_cache'
    };

    const PALETTES = {
        ufc: { name: "UFC Clássico", primary: '#0e2c4e', hover: '#1a416e', overlay: 'rgba(14, 44, 78, 0.9)', text: '#0e2c4e' },
        rocket: { name: "Rocketseat", primary: '#8257e6', hover: '#996DFF', overlay: 'rgba(130, 87, 230, 0.9)', text: '#8257e6' },
        matrix: { name: "Matrix", primary: '#059669', hover: '#10b981', overlay: 'rgba(6, 78, 59, 0.9)', text: '#064e3b' },
        orange: { name: "Sunset", primary: '#ea580c', hover: '#f97316', overlay: 'rgba(154, 52, 18, 0.9)', text: '#c2410c' }
    };
    ...
    function injectCSS() {
        GM_addStyle(`
            ...
            body { background: linear-gradient(var(--theme-overlay), var(--theme-overlay)), url('https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=2070&auto=format&fit=crop'); background-size: cover; background-position: center; display: flex; align-items: center; justify-content: center; }
            .login-card { width: 100%; max-width: 1000px; background: white; border-radius: 16px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); overflow: hidden; display: flex; flex-direction: column; opacity: 0; transform: translateY(20px); animation: fadeIn 0.6s ease-out forwards; margin: 20px; min-height: 600px; position: relative; }
            @media (min-width: 768px) { .login-card { flex-direction: row; } }
            .col-left { width: 100%; padding: 48px; display: flex; flex-direction: column; justify-content: center; background: white; z-index: 10; position: relative; }
            ...
            h2.welcome-title { font-size: 1.25rem !important; font-weight: 600 !important; color: #374151 !important; background: transparent !important; background-color: transparent !important; border: none !important; box-shadow: none !important; margin: 0 0 4px 0 !important; padding: 0 !important; text-align: left !important; text-transform: none !important; }
            ...
            .input-icon { position: absolute; top: 0; bottom: 0; left: 0; padding-left: 12px; display: flex; align-items: center; color: #9ca3af; pointer-events: none; }
            .form-input { width: 100%; padding: 12px 16px 12px 40px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 1rem; outline: none; transition: all 0.2s; background-color: #f9fafb; box-sizing: border-box; }
            .form-input:focus { border-color: var(--theme-primary); background-color: white; box-shadow: 0 0 0 3px rgba(14, 44, 78, 0.1); }
            .eye-btn { position: absolute; top: 0; bottom: 0; right: 0; padding-right: 12px; display: flex; align-items: center; color: #9ca3af; background: none; border: none; cursor: pointer; }
            .eye-btn:hover { color: #4b5563; }
            .forgot-link { display: block; text-align: right; font-size: 0.75rem; font-weight: 500; color: var(--theme-primary); text-decoration: none; margin-top: 4px; }
            .forgot-link:hover { text-decoration: underline; }
            .submit-btn { width: 100%; background-color: var(--theme-primary); color: white; font-weight: 700; padding: 12px; border-radius: 8px; border: none; cursor: pointer; transition: 0.2s; display: flex; justify-content: center; align-items: center; gap: 8px; font-size: 1rem; }
            ...
            .badge-link { font-size: 0.75rem; color: #4b5563; background: #f9fafb; border: 1px solid #e5e7eb; padding: 6px 10px; border-radius: 6px; text-decoration: none; display: flex; align-items: center; gap: 5px; transition: 0.2s; }
            .badge-link:hover { border-color: var(--theme-primary); color: var(--theme-primary); background: white; }
            .col-right { width: 100%; background-color: #f8fafc; border-left: 1px solid #e5e7eb; padding: 48px; display: flex; flex-direction: column; justify-content: center; }
            ...
            .card-link { display: flex; align-items: center; padding: 12px; background: white; border: 1px solid #e5e7eb; border-radius: 12px; text-decoration: none; transition: all 0.2s; margin-bottom: 12px; }
            ...
            .bg-blue { background: #eff6ff; color: #2563eb; } .card-link:hover .bg-blue { background: #2563eb; color: white; }
            .bg-green { background: #f0fdf4; color: #16a34a; } .card-link:hover .bg-green { background: #16a34a; color: white; }
            .bg-purple { background: #f3e8ff; color: #9333ea; } .card-link:hover .bg-purple { background: #9333ea; color: white; }
            ...
            .chip { font-size: 0.75rem; padding: 6px 12px; background: white; border: 1px solid #e5e7eb; border-radius: 6px; color: #4b5563; text-decoration: none; transition: 0.2s; }
            .chip:hover { border-color: var(--theme-primary); color: var(--theme-primary); }
            #btn-config { position: absolute; top: 15px; right: 15px; background: none; border: none; color: #9ca3af; cursor: pointer; z-index: 20; transition: 0.2s; }
    ...
    function renderLoginForm() {
        return `
            <div class="col-left">
                ...
                <form id="${SELECTORS.loginForm}">
                    <div class="input-group">
                        <label class="input-label">Usuário</label>
                        <div class="input-wrapper">
                            <span class="input-icon"><i class="ph ph-user"></i></span>
                            <input id="${SELECTORS.maskLogin}" type="text" class="form-input" placeholder="Login ou Matrícula" autocomplete="username">
                        </div>
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input id="${SELECTORS.maskSenha}" type="password" class="form-input" placeholder="Sua senha" autocomplete="current-password">
                            <button type="button" id="${SELECTORS.togglePassBtn}" class="eye-btn">
                                <i id="${SELECTORS.eyeIcon}" class="ph ph-eye" style="font-size: 1.2rem;"></i>
                            </button>
                        </div>
                        <a href="https://si3.ufc.br/admin/public/recuperar_senha.jsf" class="forgot-link">Esqueceu a senha?</a>
                    </div>
                    
                    <button type="submit" class="submit-btn" id="${SELECTORS.submitBtn}">
    ...
    function getLoginHTML() {
        return `
            <div class="login-card">
                <button id="${SELECTORS.configBtn}" title="Mudar Tema"><i class="ph ph-gear" style="font-size: 1.2rem;"></i></button>
                ${renderLoginForm()}
                ${renderRightPanel()}
            </div>
        `;
    ...
    function renderThemeButtons() {
        return Object.entries(PALETTES).map(([k, t]) => `
    ...
    function getThemeModalHTML() {
        return `
            <div class="modal-overlay" id="${SELECTORS.themeModal}">
                <div class="modal-box">
                    <button class="close-modal" id="${SELECTORS.closeModalBtn}">&times;</button>
    ...
    function handlePasswordVisibility() {
        const toggleBtn = document.getElementById(SELECTORS.togglePassBtn);
        const passInput = document.getElementById(SELECTORS.maskSenha);
        const eyeIcon = document.getElementById(SELECTORS.eyeIcon);
        
        if (!toggleBtn || !passInput || !eyeIcon) return;
    ...
    function syncOriginalForm() {
        const originalLogin = document.querySelector(SELECTORS.originalLoginForm);
        const maskLogin = document.getElementById(SELECTORS.maskLogin);
        
        if (!originalLogin || !maskLogin) return;

        setTimeout(() => {
            if(originalLogin.value && !maskLogin.value) {
    ...
    function handleFormSubmission(e) {
        ...
        const originalLogin = document.querySelector(SELECTORS.originalLoginForm);
        const originalPass = document.querySelector(SELECTORS.originalPassForm);
        const originalBtn = document.querySelector(SELECTORS.originalSubmitBtn);
        const maskLogin = document.getElementById(SELECTORS.maskLogin);
        const maskSenha = document.getElementById(SELECTORS.maskSenha);
        
        if (originalLogin && originalPass && originalBtn && maskLogin && maskSenha) {
            originalLogin.value = maskLogin.value;
            originalPass.value = maskSenha.value;
            
            const btn = document.getElementById(SELECTORS.submitBtn);
            if (btn) {
    ...
    function setupLoginInteractions() {
        ...
        const maskForm = document.getElementById(SELECTORS.loginForm);
        if (maskForm) {
            maskForm.addEventListener('submit', handleFormSubmission);
        }
    }

    function setupThemeInteractions() {
        const modal = document.getElementById(SELECTORS.themeModal);
        const btnConfig = document.getElementById(SELECTORS.configBtn);
        const btnCloseModal = document.getElementById(SELECTORS.closeModalBtn);
        ...
        document.querySelectorAll(SELECTORS.themeBtn).forEach(btn => {
            btn.onclick = () => {
                currentPalette = btn.dataset.theme;
                applyTheme();
                localStorage.setItem(CONFIG.themeKey, currentPalette);
                
                document.querySelectorAll(SELECTORS.themeBtn).forEach(b => {
    ...
    function renderComponent() {
        ...
        loginScreen.id = SELECTORS.sigaaLoginContainer;
        loginScreen.innerHTML = getLoginHTML() + getThemeModalHTML();
    ...
    init();
        ...
        carregarLogin();
    } else if (url.includes('telaAvisoLogon.jsf')) {
    ...
    init();
        ...
        carregarAviso();
    } else if (url.includes('vinculos.jsf') || url.includes('paginaInicial.do')) {
        function carregarLanding() {
            'use strict';
    ...
    function applyTheme() {
        try {
            ...
            localStorage.setItem(CONFIG.themeKey, currentPalette);
            localStorage.setItem(CONFIG.darkModeKey, String(isDarkMode));
        } catch (e) {
            console.error("Theme Error", e);
        }
    }
    ...
    function getIconName(className, text) {
        const iconMap = {
        ...
        for (const [key, icon] of Object.entries(iconMap)) {
            if (className.includes(key)) return icon;
        }

        const lowerText = text.toLowerCase();
        if (lowerText.includes('biblioteca')) return 'ph-books';
        if (lowerText.includes('rh') || lowerText.includes('pessoal')) return 'ph-users';
        if (lowerText.includes('docente')) return 'ph-chalkboard-teacher';
        if (lowerText.includes('aluno') || lowerText.includes('discente')) return 'ph-student';
    ...
    function scrapeList(selector) {
        const result = [];
        document.querySelectorAll(selector).forEach(itemElement => {
            const linkElement = itemElement.querySelector('a');
            const spanElement = itemElement.querySelector('.texto');
            ...
            result.push({
                ...
                active: !itemElement.classList.contains('off'),
                icon: getIconName(itemElement.className, text)
            });
        });
        return result;
    }
    ...
    function scrapeItems() {
        ...
        try {
            portais = scrapeList('#portais ul li');
            modulos = scrapeList('#modulos ul li');
        } catch (error) {
            console.error("Erro ao raspar dados da página inicial", error);
        }

        return {
            portais: portais.length > 0 ? portais : BACKUP_DATA.portais,
            modulos: modulos.length > 0 ? modulos : BACKUP_DATA.modulos
        };
    }
    ...
    function renderGridSection(title, iconClass, items) {
    ...
    function getLandingHTML(data) {
        ...
        return `
            ...
            ${renderHero()}
            <main class="content">
                ${renderGridSection('Outros Portais', 'ph-door-open', portaisFiltrados)}
                ${renderGridSection('Módulos do Sistema', 'ph-squares-four', data.modulos)}
            </main>
            <footer class="footer">
    ...
    function getThemeButtonsHtml() {
        return Object.entries(PALETTES).map(([key, theme]) => {
            const isSelected = currentPalette === key;
            const borderColor = isSelected ? 'var(--theme-primary)' : 'var(--border-color)';
            return `
                <button class="btn-theme" data-theme="${key}" 
                        style="background: transparent; border: 2px solid ${borderColor}; color: var(--text-primary); padding: 15px; border-radius: 12px; cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 10px; width: 100px; transition: all 0.2s;">
    ...
    function setupThemeInteractions(modal) {
    ...
    init();
        ...
        url.includes("/ava/")) && 
        document.querySelector("#agenda-docente") === null
    ) {
        function carregarTurmaVirtual() {
            'use strict';
